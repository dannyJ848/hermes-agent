...
### Method 4b: Python heredoc via terminal (AVOID for complex scripts)
When passing multi-line Python through the terminal tool, **never use shell heredocs** (`python3 << 'EOF'`). The terminal tool's shell evaluator corrupts quotes, backslashes, and f-string braces inside the heredoc body.

**Instead:** Write the script to a file first (via `write_file` or Method 4/5), then execute it:
```bash
# WRONG — heredoc gets corrupted by shell escaping
python3 << 'PYEOF'
from yantrikdb import YantrikDB  # quotes may break
batch_ids = [1, 2, 3]
placeholders = ','.join(['?'] * len(batch_ids))  # f-string braces mangled
PYEOF

# RIGHT — write to file, then execute
python3 /tmp/script.py
```

**Specific failure pattern (May 17, 2026):**
F-strings with nested quotes inside terminal heredocs produce `SyntaxError: invalid syntax` because the shell strips or rewrites the quote characters:
```python
# In terminal heredoc, this:
f'SELECT * FROM t WHERE id IN ({",".join(["?"]*len(batch_ids))})'
# Becomes corrupted to:
f'SELECT * FROM t WHERE id IN ({,.join(["?"]*len(batch_ids))})'
# → SyntaxError: invalid syntax
```

**Rule:** For any Python script longer than 2 lines or containing f-strings, nested quotes, or backslashes — write to a file via `write_file` first, then run it with `python3 /path/to/file.py`.
