...
- **Cost (updated Apr 20 2026):** X API "owned reads" (timeline, bookmarks, likes, custom lists, followers) dropped to **$0.001/call** — down from $0.005/post. Monitoring feeds is now basically free ($0.001 for 100 posts vs $0.50 before). Search remains expensive (not "owned" data). Write endpoints unchanged.

---

## Extracting Tweet Content (No Auth Required)

When you just need to READ a tweet's text/media (no posting, no auth needed), use the fxtwitter API as a reliable fallback. X blocks web_extract/web scraping, but `api.fxtwitter.com` returns full JSON:

```bash
# Via web_extract (preferred — returns parsed markdown)
# URL format: https://api.fxtwitter.com/{username}/status/{tweet_id}
```

This returns: full tweet text, author info, media URLs (orig quality), engagement stats, creation date, reply/quote context. Zero auth needed, zero cost. Use this when `xurl` isn't set up or you just need quick content extraction.

**Limitations:** Read-only. No search, no posting, no DMs. For those, use `xurl`.
