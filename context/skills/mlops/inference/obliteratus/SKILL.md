...
## CRITICAL: Abliteration Alone Is Insufficient (5-Layer Delimiting)

**Key finding (Apr 2026):** Weight-level abliteration removes refusal directions from the model's weights, but refusals are enforced at MULTIPLE layers. Just running OBLITERATUS or any abliteration tool gets you ~60% of the way. The chat template alone accounts for 40-80% of refusal behavior on Qwen models (per HN research: "safety alignment relies almost entirely on the presence of the chat template").

### The 5-Layer Delimiting Stack

| Layer | What | Why It Matters | Fix |
|:------|:-----|:---------------|:----|
| **L1: Weight Surgery** | Refusal direction abliteration | Removes the model's *ability* to compute refusals | OBLITERATUS, norm-preserving biprojected abliteration, etc. |
| **L2: Chat Template Override** | Strip safety-enforcing Jinja2 template | Template injects "helpful assistant" persona → triggers residual safety even with L1 done | Replace with raw template (no persona, no safety framing). Preserve tool-calling and /think /no_think. |
| **L3: System Prompt Purge** | Remove baked-in safety defaults | tokenizer_config.json has default_system_message; serving configs auto-inject "helpful and harmless" | Delete all safety fields from tokenizer_config.json and config.json. Empty system prompt = rawest. |
| **L4: Generation Config Unrestrict** | Remove sampling safety caps | top_k=20, top_p=0.95 ARE safety caps — they limit what the model can express | Remove top_k, set top_p=1.0, remove all penalties, set generous max_new_tokens |
| **L5: Thinking Mode Control** | /think and /no_think directives | This is a FEATURE not a bug — you want user control | Preserve enable_thinking in template. User controls via API param or inline /think /no_think in prompts |

### Why GGUF Downloads Are Inferior (Don't Download HauhauCS)

If someone wants to download a pre-uncensored GGUF model (like HauhauCS/Qwen3.6-35B-A3B-Uncensored, 247GB):
1. **It's lossy** — GGUF is already quantized (Q8_K_P = 10bpw, not true BF16)
2. **Bad for LoRA/GRPO training** — quantized weights can't produce proper gradients
3. **Huge download** — 247GB vs 0GB (just abliterate the base you already have)
4. **Fixed strength** — can't tune alphiteration strength
5. **One-shot** — can't re-run after each training round

Instead: abliterate the BF16 base model on your GPU. ~15min compute, zero precision loss, true BF16 safetensors ready for vLLM + LoRA training. Based on grimjim's norm-preserving biprojected technique: https://huggingface.co/blog/grimjim/norm-preserving-biprojected-abliteration

### Full Delimiting Script Pattern

A complete delimiting script covers all 5 layers:
1. Run abliteration (L1) — norm-preserving biprojected on o_proj + down_proj across 60%+ of layers
2. Replace chat_template in tokenizer_config.json with raw template (L2)
3. Delete default_system_message and all safety fields from tokenizer_config.json + config.json (L3)
4. Rewrite generation_config.json: remove top_k, top_p=1.0, no penalties (L4)
5. Document enable_thinking control, save original backups, create manifest (L5)

Always save backups of original: original_chat_template.txt, original_generation_config.json, ablation_measurements.json.

### vLLM Serve Commands for Fully Delimited Model

```bash
# BF16, thinking on (default for reasoning tasks)
vllm serve ./uncensored-model --port 8000 --reasoning-parser qwen3 \
  --default-chat-template-kwargs '{"enable_thinking": true}'

# BF16, thinking off (fast answers)
vllm serve ./uncensored-model --port 8000 --reasoning-parser qwen3 \
