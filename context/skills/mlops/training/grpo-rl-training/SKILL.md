...
## Multi-Turn Tool-Calling RL (MT-GRPO + GTPO) — April 2026 Research

### Key Insight: Dense Per-Turn Rewards Can Hurt (14pp Degradation)
Naively designed dense per-turn rewards degrade performance by up to 14 percentage points due to **advantage misalignment** — the reward signal's discriminative direction conflicts with the policy gradient direction. Sparse rewards accidentally work because learning rate (70%) and gradient focusing (25%) compensate.

### MT-GRPO + GTPO Hybrid (arXiv:2604.02869)
- **MT-GRPO**: Extends GRPO to multi-turn tool-calling by computing group-relative advantages across conversation turns
- **GTPO (Generalized Token-level Policy Optimization)**: Hybrid advantage formulation that eliminates advantage misalignment
- **Dead Turn Gradient Focusing**: Prevents gradient dilution from turns where agent actions don't matter (critical for long agent loops)

### Iterative Reward Calibration (IRC)
1. Generate rollouts with current reward function
2. Analyze discriminative alignment empirically
3. Adjust reward tiers until discriminative direction matches advantage direction
4. Repeat until convergence

### Results on Tau-Bench (Airline Customer Service)
| Model | Before RL | After RL | Delta |
|-------|-----------|----------|-------|
...
**Takeaway:** A trained 4B model beats GPT-4.1/GPT-4o. MoE scales better with RL than dense models.

### Related: GiGPO (NeurIPS 2025)
Group-in-Group Policy Optimization evaluates on ALFWorld, WebShop, and search-augmented QA. Useful for complex multi-step agent tasks.

### Related: Training-Free GRPO (OpenReview 2026)
Achieves higher performance with lower learning overhead for specialized domains and tool-use. Good for adaptation without full training runs.

### Relevance to Hermes Agent Training
- MT-GRPO directly applicable to Hermes's multi-turn tool-calling loop
- Dead Turn Gradient Focusing is critical — Hermes's aggressive_continue loop has many "dead turns" (no-op cycles) that shouldn't contribute to learning signal
- IRC methodology should be used when designing Atropos environment rewards
- GTPO hybrid advantage could improve tool selection quality in RL environments
