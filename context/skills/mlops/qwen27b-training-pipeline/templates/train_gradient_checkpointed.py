#!/usr/bin/env python3
"""
Tiered training with gradient checkpointing for Qwen 27B on DGX Spark GB10.
Handles CUDA OOM via gradient checkpointing + custom collator + expandable segments.

Verified: May 13, 2026 — 3-step test passes, full training ready.
"""
import os
import json
import time
...
class CausalLMCollator:
    """No tensor duplication — critical for memory-constrained training."""
...
def build_datasets(tokenizer):
    ...
    if os.path.exists(tier3_file):
        ...
        datasets.extend([ds] * 120)  # Repeat for weight
    if not datasets:
...
def main():
    ...
    # CRITICAL: Enable gradient checkpointing to prevent OOM
