...
- **Gateway module shadowing (gateway package vs hermes_cli/gateway.py)**: `hermes_cli/gateway.py` (a file module) shadows the `gateway/` package directory. When Python imports `gateway.status`, it resolves `gateway` to the file module, causing `ModuleNotFoundError: No module named 'gateway.status'; 'gateway' is not a package`. This breaks the gateway service, cron jobs, and messaging platforms. **Fix**: Pre-import the `gateway` package via `importlib.util` before any `from gateway import ...` in `hermes_cli/gateway.py`. Same fix applies to `plugins/` package if `hermes_cli/plugins.py` exists. See `references/gateway-module-shadowing-fix-2026-05-17.md`.
