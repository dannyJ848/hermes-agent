...
- **Plugin manifest (plugin.yaml)**: In addition to `__init__.py` with `register(ctx)`, a `plugin.yaml` manifest is required for Hermes to discover and load the plugin. Place it at `~/.hermes/plugins/<name>/plugin.yaml`:
  ```yaml
  name: my-plugin
  version: "1.0.0"
  description: "What this plugin does"
  author: "You"
  kind: standalone          # or backend, exclusive, platform, memory
  provides_hooks:
    - pre_tool_call
    - post_tool_call
    - on_session_start
    - on_session_end
  entry_point: "memory.yantrikdb"  # For memory providers: module path
  config:
    db_path: "~/.hermes/yantrikdb_copy.db"
    embedding_dim: 64
  ```
  The `kind: memory` tag tells Hermes this is a memory provider. The `entry_point` tells the plugin loader which module to import for `register_memory_provider()`. Without `plugin.yaml`, the plugin may exist on disk but won't be discovered by `discover_plugins()`.
- **Module shadowing by `hermes_cli` files**: When `hermes_cli/<name>.py` exists (e.g., `hermes_cli/gateway.py`, `hermes_cli/plugins.py`), it shadows the `<name>/` package directory. Python's `sys.modules` caches the first import, so `import gateway.status` resolves to the file module (no submodules) instead of the package. **Fix**: Pre-import the package via `importlib.util` before any `from <name> import ...` in `hermes_cli/<name>.py`. See `references/module-shadowing-fix.md`.
