...
Usage:
    python3 /tmp/hermes_loop_guard_v2.py <tool_name> <intent_hash> [error_msg]
...
def make_intent_hash(tool_name, intent_desc):
    """Create a hash combining tool + intent."""
...
def check_loop(tool_name, intent_desc, error_msg=None):
    ...
    # 2. Same intent 3+ times (regardless of exact command variation)
    ...
    # 4. Same tool 5+ times (absolute fallback)
    ...
    # 5. Diminishing returns: same target host 3+ SSH calls with empty/no-new-data results
    if tool_name == "terminal" and "ssh" in intent_desc.lower():
        ...
        if len(ssh_calls) >= 3:
            # Check if last 3 SSH calls all returned empty or minimal data
            last_3_ssh = ssh_calls[-3:]
            # If we can't check actual output, we use intent similarity as proxy
            ssh_intents = [c["intent_hash"] for c in last_3_ssh]
            if len(set(ssh_intents)) <= 2:  # Very similar intents
