#!/usr/bin/env python3
"""
Phase 2: Train FRANKEN v8 Draft Model — BULLETPROOF EDITION
...
9. Added gradient norm monitoring per graft
10. Added NaN detection and recovery
11. Added checkpoint integrity verification
12. Added memory usage monitoring
"""
...
class AdaptiveRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # FIX: Added clamp for numerical stability
...
def collate_fn(batch):
    ...
    # FIX: Adaptive hidden states handling
    hidden_states_list = []
    for b in batch:
        hs = b['hidden_states']
        ...
        if num_layers < 5:
            ...
            # Take first 5 layers if more than expected
            hs = hs[:5]
...
class UltimateFrankenDraftModelV8(nn.Module):
    def __init__(self, config, audit=None):
        ...
        # FIX: Adaptive fc layer based on actual target layers
        ...
        # FIX: nhead must divide hidden_size evenly
        # 5120 / 32 = 160 (integer)
        nhead = 32
        if self.hidden_size % nhead != 0:
            nhead = 16  # Fallback: 5120/16=320
            if self.hidden_size % nhead != 0:
                nhead = 8  # Ultimate fallback: 5120/8=640
    ...
    def forward(self, input_ids, target_hidden_states, attention_mask=None):
        # FIX: Null check for shared weights
        ...
        # FIX: Adaptive target hidden states handling
        actual_layers = target_hidden_states.shape[2]  # [bsz, seq_len, num_layers, hidden]
...
def parse_args():
    ...
    parser.add_argument("--learning-rate", type=float, default=3e-4)  # FIX: Reduced from 6e-4
...
def main():
    ...
    audit.info("=" * 70)
    audit.info("FRANKEN V8 — BULLETPROOF TRAINING STARTED")
    audit.info("=" * 70)
    
    config = {
        'hidden_size': 5120,
        'num_hidden_layers': args.num_layers,
        'num_attention_heads': 32,  # FIX: Changed from 24 to 32
        'num_key_value_heads': 4,
        'head_dim': 160,  # FIX: 5120/32 = 160
    ...
    audit.info(f"Loss weights: LK={args.lk_loss_weight}, DART={args.dart_loss_weight}, SSD={args.ssd_loss_weight}, LTD={args.ltd_loss_weight}")
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for batch in pbar:
            try:
                ...
                # FIX: LK Loss with detached pseudo-target
                loss_lk = 0
                if args.use_lk_loss:
                    loss_lk = lk_losses.lk_loss(
                        primary_logits[:, :-1, :],
                        primary_logits[:, :-1, :].detach(),  # FIX: Detached pseudo-target
                        temperature=1.0
                    )
                
                # FIX: DART losses with dimension guards
                loss_dart = 0
                ...
                # FIX: LTD losses with correct dimension alignment
                loss_ltd = 0
                if args.use_ltd:
                    # FIX: Align pred_tokens dimensions with shift_labels
                    pred_tokens = primary_logits.argmax(dim=-1)[..., :-1]  # Match shift_labels shape
                    correct_mask = (pred_tokens == shift_labels).float()
                    
                    # FIX: Increased epsilon for bfloat16 safety
                    eps = 1e-4
        ...
        avg_primary_loss = epoch_primary_loss / len(dataloader)
