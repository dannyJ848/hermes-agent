...
# CPU-offloaded optimizer: only keep param refs, states on CPU
...
print('\nCreating CPU-offloaded AdamW optimizer...', flush=True)
optimizer = CPUOffloadedAdamW(model.parameters(), lr=1e-5)
print(f'Optimizer param groups: {len(optimizer.param_groups)}', flush=True)

# Check memory before forward
...
# Check CPU memory for optimizer states
cpu_mem = sum(s['exp_avg'].numel() * 4 + s['exp_avg_sq'].numel() * 4 
              for group in optimizer.param_groups 
              for p in group['params'] if p in optimizer.state)
print(f'\nCPU optimizer states: {cpu_mem / 1e9:.1f}GB', flush=True)

print('\nALL TESTS PASSED - CPU-offloaded optimizer works!', flush=True)
