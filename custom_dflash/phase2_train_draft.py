...
# ============================================================
# FRANKEN GRAFT: Muon Optimizer (from DeepSeek V4)
# ============================================================
import math
    ...
    def __getitem__(self, idx):
        try:
            ...
            return {
                ...
                'hidden_states': data['hidden_states'],  # [5, seq_len, 5120]
                'seq_len': data['seq_len'],
            }
        except Exception as e:
            # Corrupted file — return a dummy sample that will be skipped
            print(f"WARNING: Corrupted file {self.files[idx]}: {e}")
            return {
                'input_ids': torch.zeros(1, dtype=torch.long),
                'hidden_states': torch.zeros(5, 1, 5120, dtype=torch.bfloat16),
                'seq_len': 1,
                '_corrupted': True,
            }
...
class DFlashDraftLayer(nn.Module):
    ...
    def forward(self, hidden_states, target_hidden, attention_mask=None):
        ...
        # Causal mask for draft generation
        # query: [bsz, num_heads, q_len, head_dim]
        # key/value: [bsz, num_heads, kv_len, head_dim]
        if attention_mask is None:
            attn_output = F.scaled_dot_product_attention(query, key, value, is_causal=True)
        else:
            # attention_mask from training loop is [bsz, 1, q_len, q_len] for self-attention
            # But here we're doing cross-attention to target_hidden, so we need causal mask for query
            # and full visibility to target keys
...
class CustomDFlashModel(nn.Module):
    def __init__(self, config):
        ...
        # Shared weights (loaded from target model)
    ...
    def forward(self, input_ids, target_hidden_states, attention_mask=None):
        # target_hidden_states: [bsz, seq_len, num_target_layers, hidden_size]
        selected = [target_hidden_states[:, :, idx, :] for idx in range(len(self.target_layers))]
        target_concat = torch.cat(selected, dim=-1)
        target_hidden = self.hidden_norm(self.fc(target_concat))
        
        hidden_states = self.embed_tokens(input_ids)
        
        for layer in self.layers:
            hidden_states = layer(hidden_states, target_hidden, attention_mask)
...
def parse_args():
    ...
    parser.add_argument("--optimizer", type=str, default="muon",
                       choices=["adamw", "muon"],
                       help="Optimizer: muon (DeepSeek V4) or adamw (baseline)")
    parser.add_argument("--muon-ns-steps", type=int, default=5,
                       help="Newton-Schulz steps for Muon")
...
def main():
    ...
    # Load ONLY embed_tokens and lm_head from target
    from transformers import AutoModelForCausalLM
    target_model = AutoModelForCausalLM.from_pretrained(
        args.target_model_path,
        torch_dtype=torch.bfloat16,
        device_map='cpu',  # Load on CPU, we'll extract weights
    ...
    print(f"Draft model parameters: {sum(p.numel() for p in draft_model.parameters()) / 1e9:.2f}B")
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for batch in pbar:
            ...
            if batch.get('_corrupted', [False])[0]:
                continue
