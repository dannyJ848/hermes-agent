...
This module monkey-patches key AIAgent methods to add:
- Semantic cache (checks cache before API calls)
- Model routing (selects optimal model per task)
- Code intelligence (injects relevant code context)
- Vector memory (semantic memory search)
- Metrics collection (API latency, tool success rates)
- Circuit breaker (already wired in model_tools.py)
- Cognitive orchestrator (runs cortex flywheel, scorecard, tool oracle)
"""
...
# (safe no-op if run_agent hasn't been imported yet)
try:
    import run_agent
