...
**`CheckpointManager.__init__() got an unexpected keyword argument 'max_total_size_mb'`**
- **Cause:** `tools/checkpoint_manager.py` `__init__` missing the `max_total_size_mb` parameter. Callers in `config.py`, `run_agent.py`, and `cli.py` pass it, but the definition only had `enabled` and `max_snapshots`.
- **Fix:** Add `max_total_size_mb: int = 500` parameter to `CheckpointManager.__init__` in `tools/checkpoint_manager.py` and store it as `self.max_total_size_mb`.
- **Verify:** `grep -n "def __init__" tools/checkpoint_manager.py` should show 4 parameters including `max_total_size_mb`.
