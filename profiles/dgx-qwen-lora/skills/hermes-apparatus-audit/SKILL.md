...
## Critical Pitfall: CLI vs Runtime Wiring

The `hermes_cli/main.py` file is a CLI wrapper — it does NOT need to import cognitive modules. The actual agent runtime is `run_agent.py`, which initializes all cognitive systems. Do NOT report cognitive systems as "unwired" just because the CLI wrapper doesn't import them.
...
- **Full apparatus wiring audit (May 18, 2026):** See `references/audit-2026-05-18-full-apparatus-wiring.md` — complete line-by-line audit of 8,033 files, discovered 7/7 cognitive system files present but **completely unwired** (zero imports, zero usage, 3,164 lines dead code). Includes tool alias vs implementation audit (76 aliases, 31 implemented, 60 unimplemented) and cross-machine sync verification (MacBook ↔ DGX). **Key distinction:** This audit found "no wiring at all" vs the May 16 audit's "partial wiring with missing post_tool_call". Also see `references/dgx-full-port-procedure-may18-2026.md` for DGX port procedure and `references/tool-alias-vs-implementation-audit-may18-2026.md` for tool count clarification.
- **Surgical integration workflow (May 18, 2026):** See `references/surgical-integration-workflow.md` — how to safely adapt upstream cherry-picked patterns into existing cognitive subsystems without replacing code. 5 patterns integrated: background review fork → training_gym, trajectory export → distillation_bridge, iteration budget → unified_intelligence, feasibility probes → context_sculptor, memory isolation → memory_bridge. Commit e2e2c5825.
