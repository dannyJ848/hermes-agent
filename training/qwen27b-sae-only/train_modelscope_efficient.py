#!/usr/bin/env python3
"""
Franken V8 + ModelScope Training (Memory-Efficient)
Loads Qwen to CPU for hidden state extraction, only Franken model on GPU.
"""
...
class FrankenV8ModelScope(nn.Module):
    def __init__(self, num_layers=6, d_model=5120, d_sae=81920, vocab_size=248320):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=d_model, nhead=16, dim_feedforward=d_model * 2,
        ...
        self.sae_aggregator = nn.Sequential(
            nn.Linear(d_sae * 2, d_sae), nn.ReLU(), nn.Linear(d_sae, d_sae)
        )
    ...
    def forward(self, input_ids, sae_features=None, labels=None):
        ...
        if sae_features is not None:
            key_layers = ['32', '48']
            aggregated = [sae_features[k] for k in key_layers if k in sae_features]
...
def precompute_sae_features(args, device):
    """Precompute SAE features for all samples and save to disk"""
    print("Loading Qwen 27B to CPU for hidden state extraction...")
    qwen_model = AutoModelForCausalLM.from_pretrained(
        args.qwen_base, torch_dtype=torch.bfloat16, device_map="cpu",
    ...
    sae_cache_dir = Path(args.output_dir) / "sae_cache"
    sae_cache_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"Precomputing SAE features for {len(samples)} samples...")
    for i, sample_path in enumerate(tqdm(samples, desc="SAE cache")):
        cache_path = sae_cache_dir / f"{sample_path.stem}_sae.pt"
        ...
        with torch.no_grad():
            ...
            if len(hidden_states) > 33:
                h32 = hidden_states[33].to(torch.bfloat16)
                sae_features['32'] = sae_32(h32)
            if len(hidden_states) > 49:
                h48 = hidden_states[49].to(torch.bfloat16)
                sae_features['48'] = sae_48(h48)
        
        torch.save(sae_features, cache_path)
    
    # Clear Qwen from memory
    del qwen_model
    del sae_32
    del sae_48
    torch.cuda.empty_cache()
    
    return sae_cache_dir, samples
...
def train():
    ...
    parser.add_argument("--skip-cache", action="store_true", help="Skip SAE precomputation (use existing cache)")
    args = parser.parse_args()
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Precompute or load SAE features
    sae_cache_dir = Path(args.output_dir) / "sae_cache"
    hs_path = Path(args.hidden_states_dir)
    samples = sorted(hs_path.glob("sample_*.pt"))
    
    if not args.skip_cache or not sae_cache_dir.exists():
        sae_cache_dir, samples = precompute_sae_features(args, device)
    
    # Build model
    print("Building Franken V8 + ModelScope model...")
    model = FrankenV8ModelScope(
        num_layers=6, d_model=5120, d_sae=81920, vocab_size=248320
    ...
    while step < args.num_steps:
        for sample_path in samples:
            ...
            # Load precomputed SAE features
            cache_path = sae_cache_dir / f"{sample_path.stem}_sae.pt"
            sae_features = torch.load(cache_path, map_location=device)
