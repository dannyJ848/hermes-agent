...
class GatedAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, is_bidirectional=False):
        ...
        k = k + 0.1 * lookahead_k
        v = v + 0.1 * lookahead_v
        attn_output = F.scaled_dot_product_attention(q, k, v, attn_mask=attention_mask, is_causal=not is_bidirectional, scale=self.scaling)
...
class GraftManager:
    ...
    WAVES = [
        ...
        {
            'name': 'Speculation Heavy (SSD + DART + LTD)',
            'grafts': {'mtp4': False, 'pard': False, 'tree_attn': False, 'early_exit': False, 'bidirectional': False},
...
def train(args):
    ...
    # Move core model to GPU
    model = model.to(device)
    if args.bf16:
        model = model.to(torch.bfloat16)
        logger.info("Using bfloat16")
    
    # Setup GraftManager for progressive loading
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if (batch_idx + 1) % args.grad_accum == 0:
                ...
                if global_step % args.save_interval == 0:
                    ...
                    torch.save({'step': global_step, 'epoch': epoch, 'model_state_dict': model.state_dict(),
                                'optimizer_state_dict': optimizer.state_dict(), 'scheduler_state_dict': scheduler.state_dict()}, checkpoint_path)
